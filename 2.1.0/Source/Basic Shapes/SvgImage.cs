        ...
        protected object GetImage(string uriString)
        {
            try
            {
                ...
                if (uriString.StartsWith("data")) 
                {
                    ...
                    string imageType = uriString.Substring(0, dataIdx); // Check for SVG embedded in the data...
                    if (imageType.Contains("image/svg"))
                        {
                            string embeddedSVG;
                            if (imageType.Contains("base64") )
                            {
                                embeddedSVG = System.Text.Encoding.UTF8.GetString( Convert.FromBase64String(uriString.Substring(dataIdx)) );
                            }
                            else
                            {
                                embeddedSVG = uriString.Substring(dataIdx);
                            }
                            using (var stream = new MemoryStream(System.Text.UTF8Encoding.Default.GetBytes(embeddedSVG)))
                            {
                                var doc = SvgDocument.Open<SvgDocument>(stream);
                                doc.BaseUri = new Uri(uriString);
                                return doc;
                            }
                        }
                    else
                        {
                            ...
                            using (var stream = new MemoryStream(imageBytes))
                            {
                                ...
                                //Make a copy of the image and return the copy.
                                //Do not return the image gotten from the stream because, after the stream is closed,
                                //the image cannot be used.
                                var result = new Bitmap(sourceImage.Width, sourceImage.Height, sourceImage.PixelFormat);
